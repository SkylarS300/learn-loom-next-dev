...
-- AlterTable
ALTER TABLE `assignmentcompletion` ADD COLUMN `attemptCount` INTEGER NOT NULL DEFAULT 0,
    ...
    ADD COLUMN `status` ENUM('ASSIGNED', 'SUBMITTED', 'GRADED', 'MISSING', 'LATE') NOT NULL DEFAULT 'ASSIGNED',
    ADD COLUMN `submittedAt` DATETIME(3) NULL;

-- AlterTable
ALTER TABLE `studentclassroom` ADD COLUMN `displayName` VARCHAR(191) NULL;
...
-- AddForeignKey
ALTER TABLE `assignmenttarget` ADD CONSTRAINT `assignmenttarget_assignmentId_fkey` FOREIGN KEY (`assignmentId`) REFERENCES `assignment`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
