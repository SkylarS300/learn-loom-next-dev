...
export async function POST(req) {
    ...
    try {
        const w = await prisma.word.upsert({
            where: { lemma_pos: { lemma: (lemma || word).toLowerCase(), pos: "unknown" } },
            update: { display: word },
            create: { lemma: (lemma || word).toLowerCase(), display: word, pos: "unknown", cefrLevel: "UNKNOWN" }
        });
        await prisma.wordEncounter.create({
            data: { anonId, wordId: w.id, source: String(source || "reading").toUpperCase(), context: context.slice(0, 240) }
        });
