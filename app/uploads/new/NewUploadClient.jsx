// app/uploads/new/NewUploadClient.jsx
"use client";

import { useEffect, useState, useRef } from "react";
import { useRouter } from "next/navigation";

export default function NewUploadClient() {
    // ---- limits ----
    ...
    async function handleSubmit(e) {
        ...
        if (!title.trim()) { setLoading(false); setError("Please enter a title."); return; }
        ...
        if (res.ok) {
            ...
            if (newId) router.push(`/uploads/${newId}`);
            else setError("Upload succeeded but response had no ID.");
        } else {
    ...
    function handleFileUpload(e) {
        ...
        if (!isPdf && !isTxt) { setError("Only PDF and TXT files are supported."); return; }
        if (isPdf && file.size > MAX_PDF_BYTES) { setError("PDF is too large. Max 12 MB."); return; }
        if (isTxt && file.size > MAX_TXT_BYTES) { setError("TXT is too large. Max 1 MB."); return; }
        ...
        if (isPdf) {
            ...
            reader.onload = async () => {
                try {
                    ...
                    console.error(err);
                    setError("Could not read PDF. Try a text-based PDF or upload a TXT file.");
                }
            };
        } else if (isTxt) {
            ...
            reader.onload = () => {
                try {
                    ...
                    if (new Blob([txt]).size > MAX_TXT_BYTES) { setError("TXT is too large. Max 1 MB."); return; }
    ...
    return (
        <section className="upload-form" style={{ maxWidth: "700px", margin: "0 auto" }}>
            ...
            <div style={{ marginBottom: "1rem" }}>
                <button type="button" onClick={() => setMode("typed")} className={mode === "typed" ? "active-tab" : ""}>
                    📝 Type Text
                </button>
                <button type="button" onClick={() => setMode("file")} className={mode === "file" ? "active-tab" : ""}>
                    📁 Upload File
                </button>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                {mode === "file" && (
                    <>
                        <label>
                            ...
                            <input type="file" accept=".txt,application/pdf" onChange={handleFileUpload} required />
                        </label>
                        {fileName && <p>📄 <strong>{fileName}</strong> selected</p>}
                        {fileContent && showPreview && (
                            <pre style={{ background: "#f9f9f9", border: "1px solid #ccc", padding: "1rem", maxHeight: 200, overflowY: "auto", whiteSpace: "pre-wrap" }}>
                ...
                <label>
                    ...
                    <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Leave blank to keep unlocked" />
                </label>
        ...
        button[type="submit"] { background: #0070f3; color: white; border: none; padding: 0.75rem; border-radius: 6px; font-size: 1rem; cursor: pointer; }
      `}</style>
        </section>
    );
}
