...
export default function Navbar() {
  // --- single-instance guard (all hooks must be top-level, unconditional) ---
  const [hidden, setHidden] = useState(false);
  const isPrimaryRef = useRef(false);

  // ⚠️ Declare ALL hooks before any return
    ...
    <header className={styles.navbar} role="navigation" data-role="site-navbar">
      <div className={styles.inner}>
        ...
        <div className={styles.actions} style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: "auto" }}>
          {loading ? (
            <div aria-busy="true" aria-live="polite" style={{ color: "#6b7280", fontSize: 12 }}>Loading…</div>
          ) : loggedIn ? (
            <>
              <button onClick={copyCode} className={styles.btnSecondary} style={btnSecondary}>Copy code</button>
              {me.shortCode && (
                <button onClick={() => setShowQR(true)} className={styles.btnSecondary} style={btnSecondary}>Show QR</button>
              )}
